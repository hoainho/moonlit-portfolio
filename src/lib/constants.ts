export const siteConfig = {
  name: "Bé Dương",
  title: "Bé Dương | Graphic Designer",
  description: "Portfolio of Bé Dương - Graphic Designer specializing in Visual Design, Brand Identity, and Event Graphics. Where dreams take shape under the moonlight.",
  tagline: "Where dreams take shape",
  url: "https://beduong.art",
  
  navigation: [
    { name: "Home", href: "#home" },
    { name: "About", href: "#about" },
    { name: "Works", href: "#works" },
    { name: "Skills", href: "#skills" },
    { name: "Contact", href: "#contact" },
  ],
  
  social: {
    email: "Nguyenbeduong2004@gmail.com",
    linkedin: "https://www.linkedin.com/in/be-duong/",
    behance: "https://www.behance.net/bdng3",
    artstation: "#",
    instagram: "#",
  },
  
  about: {
    intro: "Hello, I'm Dương",
    description: "A young graphic designer with a career focus on visual design and brand identity. Passionate about visual storytelling, proactive in learning, and open to feedback to improve skills in a professional work environment.",
    quote: "I believe every visual tells a story, and I'm here to tell yours.",
    education: {
      school: "Hutech University",
      major: "Graphic Design",
      year: "2020 - 2024",
      gpa: "3.2/4"
    },
    funFacts: [
      { emoji: "🎨", label: "Designer" },
      { emoji: "✨", label: "Visual Storyteller" },
      { emoji: "🌙", label: "Night Owl" },
    ]
  },
  
  skills: {
    tools: [
      { name: "Photoshop", icon: "Ps" },
      { name: "Illustrator", icon: "Ai" },
      { name: "InDesign", icon: "Id" },
      { name: "Figma", icon: "Fg" },
      { name: "After Effects", icon: "Ae" },
    ],
    capabilities: [
      { emoji: "🎨", label: "Visual Design" },
      { emoji: "🏷️", label: "Brand Identity" },
      { emoji: "🎪", label: "Event Graphics" },
      { emoji: "✏️", label: "Typography" },
      { emoji: "🖼️", label: "Illustration" },
      { emoji: "📦", label: "Packaging Design" },
    ]
  }
};

export const portfolioItems = [
  {
    id: 1,
    title: "Hồn Sắc Nam Bộ",
    category: "Event Design",
    image: "/dieu-am-vang.png",
    description: "Graduation Project - Event Graphics System for the Southern Handicraft Village Exhibition & Conference. Developed a comprehensive visual identity system reflecting Southern Vietnam's cultural heritage.",
    tools: ["Photoshop", "Illustrator", "InDesign", "After Effects"],
    year: "2024"
  },
  {
    id: 2,
    title: "Tiêu Kỳ",
    category: "Event Design",
    image: "/tue-ky.png",
    description: "Key Visual Event Design System. Developed main visuals, logo, brand identity, posters, banners, backdrops, tickets, and social media content ensuring visual consistency across all platforms.",
    tools: ["Photoshop", "Illustrator"],
    year: "2024"
  },
  {
    id: 3,
    title: "Nhà Ngoại",
    category: "Brand Identity",
    image: "/nha-ngoai.png",
    description: "Complete Brand Identity Design including logo, color palette, typography, graphic elements, business cards, packaging, and social media visuals with comprehensive brand guidelines.",
    tools: ["Illustrator", "Photoshop", "InDesign"],
    year: "2024"
  },
  {
    id: 4,
    title: "Fresh Fruit",
    category: "Brand Identity",
    image: "/fresh-fruit.png",
    description: "Beverage Brand Identity Design featuring logo, visual identity system, packaging labels, cups, bottles, menus, promotional materials, and in-store imagery.",
    tools: ["Illustrator", "Photoshop"],
    year: "2024"
  },
  {
    id: 5,
    title: "Dragon Serif",
    category: "Typography",
    image: "/dragon.png",
    description: "Typography Design Project exploring creative typographic layouts, experimental letterforms, spacing, grid systems, and expressive conceptual typography with strong visual impact.",
    tools: ["Illustrator", "Photoshop"],
    year: "2024"
  },
  {
    id: 6,
    title: "Tứ Phủ",
    category: "Illustration",
    image: "/tu-phu.png",
    description: "Team Publication Project with 'Four Palaces' cultural theme. Created and refined illustrative elements, supported layout design with supplementary illustrations for typography and page layout.",
    tools: ["InDesign", "Illustrator", "Photoshop"],
    year: "2024"
  },
  {
    id: 7,
    title: "Cô Bé Quàng Khăn Đỏ",
    category: "Illustration",
    image: "/co-be-quang-khan-do.png",
    description: "Basic Illustration Project for the story 'Little Red Riding Hood'. Developed character sketches, key highlights, applied color, composition, and visual storytelling techniques.",
    tools: ["Photoshop", "Illustrator"],
    year: "2024"
  },
];
